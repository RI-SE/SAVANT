...
Core Arguments:
    --config, -c       Load parameters from JSON config file (generated by trainit-gui)
    --data, -d         Dataset configuration file (YAML) (required unless in config)
...
class YOLOTrainer:
    ...
    def print_training_info(self) -> None:
        ...
        if self.config.flipud is not None:
            aug_params.append(f"FlipUD: {self.config.flipud}")
...
def load_config_file(config_path: str) -> dict:
    """Load training parameters from a JSON config file.

    Config files are generated by trainit-gui and contain training parameters.
    Metadata fields (starting with '_') are ignored.
...
def merge_config_with_args(args: argparse.Namespace, config: dict) -> argparse.Namespace:
    ...
    Only updates args that are at their default values.
    ...
    for key, value in config.items():
        ...
        # For parameters with defaults, only override if CLI matches default
        if attr_name in defaults:
            if current_value == defaults[attr_name]:
                setattr(args, attr_name, value)
                logger.debug(f"Using config value for {attr_name}: {value}")
        else:
            # For optional parameters (default None), only override if not set
...
def build_arguments_string(args: argparse.Namespace) -> str:
    ...
    if args.flipud is not None:
        parts.append(f"--flipud {args.flipud}")
    ...
    try:
        ...
        if args.config:
            try:
                ...
                args = merge_config_with_args(args, config_data)
