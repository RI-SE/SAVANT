...
class TrainingConfig(BaseModel):
    ...
    selected_datasets: list[str] = Field(
        default_factory=list, description="List of dataset folder names to include"
    )
    ...
    imgsz: int = Field(
        default=640, ge=32, le=2048, description="Image size for training"
    )
    batch: int = Field(default=30, ge=1, description="Batch size")
    device: str = Field(
        default="auto", description="Device: 'cuda', 'mps', 'cpu', or 'auto'"
    )
    project: str = Field(
        default="runs/obb", description="Project directory for saving results"
    )

    # Advanced training parameters (None = use YOLO defaults)
    lr0: Optional[float] = Field(
        default=None, gt=0, description="Initial learning rate"
    )
    lrf: Optional[float] = Field(
        default=None, ge=0, le=1, description="Final learning rate as fraction of lr0"
    )
    optimizer: Optional[
        Literal["SGD", "Adam", "AdamW", "NAdam", "RAdam", "RMSProp"]
    ] = Field(default=None, description="Optimizer choice")
    warmup_epochs: Optional[float] = Field(
        default=None, ge=0, description="Number of warmup epochs"
    )
    warmup_momentum: Optional[float] = Field(
        default=None, ge=0, le=1, description="Initial momentum during warmup"
    )
    patience: Optional[int] = Field(
        default=None, ge=1, description="Early stopping patience"
    )
    save_period: Optional[int] = Field(
        default=None, description="Save checkpoint every N epochs (-1 to disable)"
    )
    cache: Optional[Literal["true", "false", "ram", "disk"]] = Field(
        default=None, description="Image caching strategy"
    )
    workers: Optional[int] = Field(
        default=None, ge=0, description="Number of dataloader workers"
    )
    close_mosaic: Optional[int] = Field(
        default=None, ge=0, description="Disable mosaic in final N epochs"
    )
    freeze: Optional[int] = Field(
        default=None, ge=0, description="Freeze first N layers"
    )

    # Loss weights
    box: Optional[float] = Field(default=None, gt=0, description="Box loss gain weight")
    cls: Optional[float] = Field(
        default=None, gt=0, description="Classification loss gain"
    )
    dfl: Optional[float] = Field(
        default=None, gt=0, description="Distribution focal loss gain"
    )

    # Augmentation parameters
    hsv_h: Optional[float] = Field(
        default=None, ge=0, le=1, description="HSV Hue augmentation range"
    )
    hsv_s: Optional[float] = Field(
        default=None, ge=0, le=1, description="HSV Saturation augmentation range"
    )
    hsv_v: Optional[float] = Field(
        default=None, ge=0, le=1, description="HSV Value augmentation range"
    )
    degrees: Optional[float] = Field(
        default=None, ge=0, description="Rotation augmentation in degrees"
    )
    translate: Optional[float] = Field(
        default=None, ge=0, le=1, description="Translation augmentation fraction"
    )
    scale: Optional[float] = Field(
        default=None, gt=0, description="Scaling augmentation gain"
    )
    shear: Optional[float] = Field(
        default=None, ge=0, description="Shear augmentation in degrees"
    )
    perspective: Optional[float] = Field(
        default=None, ge=0, description="Perspective augmentation"
    )
    fliplr: Optional[float] = Field(
        default=None, ge=0, le=1, description="Horizontal flip probability"
    )
    flipud: Optional[float] = Field(
        default=None, ge=0, le=1, description="Vertical flip probability"
    )
    mosaic: Optional[float] = Field(
        default=None, ge=0, le=1, description="Mosaic augmentation probability"
    )
    mixup: Optional[float] = Field(
        default=None, ge=0, le=1, description="Mixup augmentation probability"
    )

    # Train/Val split configuration (used during generation, not training)
    split_enabled: bool = Field(
        default=False, description="Enable train/val re-mapping during generation"
    )
    ...
    split_seed: int = Field(
        default=42, ge=0, description="Random seed for reproducible splits"
    )
...
class Project(BaseModel):
    ...
    default_config: Optional[ProjectDefaults] = Field(
        default=None, description="Default training parameters for new configurations"
    )

    # Training configurations
    configs: list[TrainingConfig] = Field(
        default_factory=list, description="List of training configurations"
    )
