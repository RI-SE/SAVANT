...
class TestInterpolateCenterTrajectory:
    ...
    @pytest.mark.parametrize(
        "start,end,frames,expected_length",
        [
    ...
    def test_interpolate_center_trajectory_basic(
        self, start, end, frames, expected_length
    ):
...
class TestInterpolateAnnotations:
    ...
    def test_interpolate_annotations_basic(self, sample_bbox):
        ...
        assert all(
            set(bbox.keys()) == {"x_center", "y_center", "width", "height", "rotation"}
            for bbox in result
        )
        ...
        assert np.allclose(
            np.diff(widths),
            (end_bbox["width"] - start_bbox["width"]) / (num_frames - 1),
        )
