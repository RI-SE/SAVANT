# Savant Edit Documentation

## Audience & Scope
This document covers everything you need to run the Savant desktop annotator (`edit`) and everything you need to maintain or extend it. It is split into two parts:
...
Use the uv workflows documented in `edit/README.md`:
```bash
# Run the entire pytest suite (unit tests live under edit/tests)
uv run pytest
...
1. Start from `edit/src/edit`.
2. Run the packaged entry point:
   ```bash
   uv run python -m edit.main
   ```
3. On launch the app:
   - Sets up rotating log handlers (`edit.log` alongside an OS-specific log dir).
...
- Logs are written both to `edit.log` (repo root) and the OS log directory retrieved via `appdirs.user_log_dir`.
...
- The architecture follows clean layering:
  1. **Frontend (PyQt6)** – widgets, menus, playback controls, overlay (under `edit/src/edit/frontend`).
...
### 2.8 Testing & QA
- Unit tests live under `edit/tests/unit`, grouped by layer:
...
2. Add or update documentation (this file, `edit/README.md`, or doc images) when introducing meaningful feature changes.
...
For quick reference, keep `edit/README.md` for onboarding commands, and use `docs/edit.md` (this file) as the canonical in-repo handbook for both users and contributors.